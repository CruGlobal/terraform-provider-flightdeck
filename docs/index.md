...
description: |-
  The flightdeck provider manages a Flightdeck workspace's project configuration — projects (including their self-healing rollback configuration), workflow states, labels, members, GitHub repository links, ingestion tokens, error and incident alert rules, Events API routing keys, PagerDuty links and outbound webhooks — through its REST API, so that configuration can live in Terraform alongside the rest of an application's infrastructure. Flightdeck is a project-management application (workspaces, projects, work items) with built-in error tracking and incident management.
  Authentication
