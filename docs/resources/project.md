...
- `name` (String) Channel name to use instead of the project-name default. Flightdeck normalizes it to Slack's rules (lower case, runs of other characters become `-`, at most 80 characters), so `My Team!` is stored as `my-team`; the configured spelling is kept in state and does not diff. When unset the project's current value is kept, so **set it to `""` to clear an override** and fall back to the project-name default — removing the attribute leaves whatever is there. Empty means no override, which is what `basename` then reports. Changing the effective name drops the stored `channel_id` so the next provision links the new channel; the old one is left on Slack.
