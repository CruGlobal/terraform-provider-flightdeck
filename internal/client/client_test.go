...
func TestCreateWithReplayGuard(t *testing.T) {
	ctx := context.Background()
	notFoundErr := &Error{Status: 404, Code: CodeNotFound}

	t.Run("fresh create passes through", func(t *testing.T) {
		var keys []string
		got, err := CreateWithReplayGuard(ctx, "stable",
			func(_ context.Context, key string) (int, error) { keys = append(keys, key); return 42, nil },
			func(_ context.Context, id int) error { return nil })
		if err != nil || got != 42 || len(keys) != 1 || keys[0] != "stable" {
			t.Fatalf("got=%d err=%v keys=%v", got, err, keys)
		}
	})

	t.Run("stale replay is retried with a fresh key", func(t *testing.T) {
		var keys []string
		got, err := CreateWithReplayGuard(ctx, "stable",
			func(_ context.Context, key string) (int, error) {
				keys = append(keys, key)
				if key == "stable" {
					return 1, nil // the replayed, since-deleted resource
				}
				return 2, nil
			},
		...
		if len(keys) != 2 || keys[0] != "stable" || keys[1] == "stable" {
			t.Errorf("keys = %v", keys)
		}
	})

	t.Run("other verification errors surface", func(t *testing.T) {
		wantErr := &Error{Status: 403, Code: CodeForbidden}
		_, err := CreateWithReplayGuard(ctx, "stable",
			func(_ context.Context, key string) (int, error) { return 1, nil },
			func(_ context.Context, id int) error { return wantErr })
	...
	t.Run("create errors surface", func(t *testing.T) {
		wantErr := &Error{Status: 422, Code: CodeValidationFailed}
		_, err := CreateWithReplayGuard(ctx, "stable",
			func(_ context.Context, key string) (int, error) { return 0, wantErr },
			func(_ context.Context, id int) error { t.Fatal("verify must not run"); return nil })
		if !errors.Is(err, wantErr) {
			t.Fatalf("err = %v", err)
		}
	})
